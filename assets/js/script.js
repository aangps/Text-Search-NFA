$(document).ready(function () {
  ...
  // FUNGSI UTAMA KETIKA TOMBOL SUBMIT DI-KLIK ATAU ENTER
  ...
  // FUNCTION AMBIL DATA DARI DOKUMEN KE VARIABLE -> Parameter = Nomer Dokumen yang mau diakses -> Return Value = Number Dokumen, Isi Dokumen
  function getDataFromDoc(numberDoc) {
    let link = ''.concat('assets/DokumenBasaBali/doc',numberDoc,'.txt');
    var doc = {
      data: '',
      number: numberDoc
    };
  ...
  function printQuintupleNFA(a,b,c,d,e) {
    $('#quintupleNFA').html("<h6>Hasil Generate Mesin NFA (Quintuple) berdasarkan input user :</h6><p>Set of state : {"+a+"}</p><p>Set of symbol : {"+b+"}</p><p>Initial State : "+c+"</p><p>Set of final state : {"+d+"}</p><p>Trantition Function : </p>");
    for(let key in e){
      for(let keys in e[key]){
        $('#quintupleNFA').append("<p><pre>     Delta("+key+","+keys+") = "+e[key][keys]+"</pre></p>");
      }
    }
  }
  
  // LOOPING PENGECEKAN KATA INPUT DISETIAP FILE DOKUMEN
  function checkDocument(){
    // Varibel untuk menyimpan data dokumen mana saja yang mengandung kata input
    var searchResult = [];
  ...
  // FUNGSI PRINT HASIL SEARCH + SNIPPET DOCUMENT
  function printResultDocument(arrayNum){
    $('#resultDoc').html("<h6>Hasil Pencarian : </h6>");
    let arrayNum2 = Array.from(new Set(arrayNum));
    for(let i=0; i<arrayNum2.length; i++){
      let p = (getSnippet(arrayNum2[i]));
      $('#resultDoc').append('<a href="assets/DokumenBasaBali/doc'+arrayNum2[i]+'.txt" target="_blank">doc'+arrayNum2[i]+'.txt</a>'+'<p>'+p+'</p>');
    }
  }
  ...
  // Function Bantuan untuk memasukkan nested key dan value kedalam object secara dinamis
